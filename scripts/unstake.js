...
async function main() {
  ...
  for (let i = 0; i < signers.length; i++) {
    ...
    await staking.connect(signer).unstake(shard.address, stakingShards);
    await staking.connect(signer).unstake(citizen.address, stakingCitizens);
    await staking.connect(signer).unstake(land.address, stakingLands);
  }
